        ...
        if ClientConfig.ClientTriggers then
            for _,cliev in pairs(ClientConfig.ClientTriggerList) do
                RegisterNetEvent(cliev, function()
                    TriggerServerEvent("bltkac_detection", "Client Trigger", "Client-side trigger used\n**Trigger:** `"..cliev.."`", ClientConfig.ClientTriggerKick, ClientConfig.ClientTriggerBan) 
                end)
            end
        end
            ...
            Citizen.CreateThread(function()
                ...
                while true do
                    ...
                    if ClientConfig.AntiAFKBypass then
                        if GetIsTaskActive(PlayerPedId(), 100) or GetIsTaskActive(PlayerPedId(), 101) or GetIsTaskActive(PlayerPedId(), 151) or GetIsTaskActive(PlayerPedId(), 221) or GetIsTaskActive(PlayerPedId(), 222) then
                            TriggerServerEvent("bltkac_detection", "MenuCheck AntiAFK Bypass", "This player tried to bypass AntiAFK.", ClientConfig.MenuCheckKick, ClientConfig.MenuCheckBan) 
                        end
                    end
                    if ClientConfig.Styles then
                        if HasStreamedTextureDictLoaded('fm') or HasStreamedTextureDictLoaded('rampage_tr_main') or HasStreamedTextureDictLoaded('MenyooExtras') then
                            TriggerServerEvent("bltkac_detection", "MenuCheck Styles", "Mennyoo/Rampage based menu detected.", ClientConfig.MenuCheckKick, ClientConfig.MenuCheckBan) 
                        end
                        if HasStreamedTextureDictLoaded('shopui_title_graphics_franklin') or HasStreamedTextureDictLoaded('deadline') then
                            TriggerServerEvent("bltkac_detection", "MenuCheck Styles", "Dopamine based menu detected.", ClientConfig.MenuCheckKick, ClientConfig.MenuCheckBan) 
                        end
                        if HasStreamedTextureDictLoaded('cockmenuuu') then
                            TriggerServerEvent("bltkac_detection", "MenuCheck Styles", "CockMenu based menu detected.", ClientConfig.MenuCheckKick, ClientConfig.MenuCheckBan) 
                        end
                    end
            ...
            if data.text ~= nil then     
                ...
                for _,word in pairs(badwords) do
                    if string.find(string.lower(data.text), string.lower(word)) then
                        TriggerServerEvent("bltkac_detection_ai", data.screenshoturl, "AI", "AI Detected a suspicious word on the screen of the player. \n**Word:** `"..word.."`", ClientConfig.InjectKick, ClientConfig.InjectCheckBan) 
                    end
                end
            end
        end)
                ...
                while true do
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        ...
                        if GetVehicleTopSpeedModifier(vehicle) > 1.0 then
                            ...
                            DeleteEntity(vehicle)
                            TriggerServerEvent("bltkac_detection", "MenuCheck Vehicle Modifiers", "This player tried to enable vehicle modifiers/vehicle cheats", ClientConfig.MenuCheckKick, ClientConfig.MenuCheckBan) 
                        end
                    else
                        Citizen.Wait(3000)
                    end
